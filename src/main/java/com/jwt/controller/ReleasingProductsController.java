...
import com.jwt.model.CoverModel;
...
public class ReleasingProductsController {
	...
	@RequestMapping(value="/TestDownloadApi")
	public void test_api()
	{
		ApiClient apiClient = new ApiClient();
			apiClient.callOracleApiForRelease("21420", "2020-07-21");
	}
...
//	        }
	...
	public ModelAndView coverLists(ModelAndView model,HttpServletRequest request, HttpServletResponse response ,@RequestParam(value = "Username", required = true) String Username )throws ServletException, IOException {
    	...
    	List<CoverModel>covermod=new ArrayList<CoverModel>();
    	for(int i=0;i<covers_lst.size();i++)
    	{
    		CoverModel cov=new CoverModel();
    		cov.setDriver_id(covers_lst.get(i).getDriver_id());
    		cov.setAssignment_date(covers_lst.get(i).getAssignment_date());
    		cov.setDriver_name_ar(covers_lst.get(i).getDriver_name_ar());
    		cov.setDriver_name_en(covers_lst.get(i).getDriver_name_en());
    		Date x_date=java.sql.Date.valueOf(covers_lst.get(i).getAssignment_date());
    		DateFormat format=new SimpleDateFormat("dd-MM-yyyy");
    		cov.setAssign_date(format.format(x_date));
    		covermod.add(cov);
    		
    	}
    	newMod.addObject("covers_lst",covermod );
		return newMod;
	}
	...
	//	List<UserTeam> listTeam = userTeamService.getAllUserTeam(user);
		...
		Date ass_date=java.sql.Date.valueOf(Assignment_date);
		DateFormat format=new SimpleDateFormat("dd-MM-yyyy");
		newMod.addObject("assignment_date", Assignment_date);
		newMod.addObject("assign_date", format.format(ass_date));
		
		newMod.addObject("productsLst",coverForm);
		newMod.addObject("Username", u.getDisplay_name()+Username);
	...
	public ResponseEntity<String> submitCoverDetails(@RequestBody  ProductListsCoverForm coverForm)throws ServletException, IOException {
		...
		//	List<UserTeam> listTeam = userTeamService.getAllUserTeam(user);
			...
			//	apiClient.DownloadDispatchedToOperationById(shipment_id);
				return new ResponseEntity<String>("success", HttpStatus.OK);
