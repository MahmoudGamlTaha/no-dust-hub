...
public class ReleaseCashController {
	...
	public ModelAndView login(ModelAndView model , HttpServletRequest request) throws IOException {
		...
		List<Drivers_Users>lst_drivers_users=cashInoutService.get_user_names();
		cashInOutForm.setLst_drivers_users(lst_drivers_users);
	...
	public  ResponseEntity<String> submitCashInOut(@RequestBody CashInOutForm cashInOutForm)throws ServletException, IOException {
		...
		//	List<UserTeam> listTeam = userTeamService.getAllUserTeam(user);
			...
			{
				submited=cashInoutService.submitCashInOut(cashInOutForm.getSelected_product_id(), u.getUSER_NAME(),cashInOutForm.getRecievedCash(),cashInOutForm.getSelectedDriver(),cashInOutForm.getComment());	
