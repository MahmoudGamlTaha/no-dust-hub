...
public class LoginController {
	...
	@Autowired
	private DMSUsersService dmsusersservice;
	//@Autowired
...
//	private UserTeamService userTeamService;
	...
	public ModelAndView upload_img(ModelAndView newMod)
	{
		DMS_Users x_image=dmsusersservice.Get_user_image("12003");
		System.out.println(x_image.getImage());
		List<DMS_Users>lst_drivers=dmsusersservice.get_list_users();
		List<DMS_DriversData>lst_data=dmsusersservice.Check_image_flag();
		newMod.addObject("lst_drivers", lst_drivers);
		newMod.addObject("image", x_image);
		newMod.addObject("data", lst_data);
		newMod.setViewName("SaveImg");
		return newMod;
	}
	@RequestMapping(value="/save_img",method=RequestMethod.POST, consumes = "application/json")
	public ResponseEntity<String> Save_Img(@RequestBody DMS_Users User_Data_form) throws FileNotFoundException
	{
		DMS_Users User_Data=null;
		System.out.println("User_name"+User_Data_form.getUser_name());
		System.out.println("Image"+User_Data_form.getImage());
		System.out.println("Save_img_fun");
		//File file=new File(User_Data.getImg());
		//System.out.println(file);
		String out=dmsusersservice.Update_img(User_Data_form);
	...
	public ModelAndView login(HttpServletRequest request,@RequestParam(value = "error", required = false) String error,
		...
		if (error2 != null)
		{
			model.addObject("error", "Another User is logged in");
...
session.setAttribute("fac", fac.getFac_name());
		...
		return new ResponseEntity<String>(fac.getFacility_id(),HttpStatus.OK);
	}
	...
	//	List<UserTeam> listTeam = userTeamService.getAllUserTeam(user);
	    ...
	    userService.updateUser(u);
		   ...
		   System.out.println("Date Now : "+now);
		    ...
		    {
		    	ModuleSession mod_session_var=userService.get_session_timeout(1);
		    	if(mod_session_var!=null)
		    	{
		    	
				request.getSession().setMaxInactiveInterval(mod_session_var.getSession_duration()*60);
		    	}
		    	System.out.println("Session_Timeout"+request.getSession().getMaxInactiveInterval());
	...
	public ResponseEntity<String> Login_User(HttpServletRequest request , @RequestBody UserData user_data)
	{
		...
		if(user_info.getActive()==0)
		{
			...
			String x_val1=userService.Logout_user(user_info.getUser_name());
		...
		return new ResponseEntity<String>("true",HttpStatus.OK);
		   ...
		   if(user_info.getActive()==1)
		   {
			 ...
			 if(check_var==false)
			 {
					...
					String x_val2=userService.Update_Login_Time(user_name);
					return new ResponseEntity<String>("true",HttpStatus.OK);
