...
public class CreateDirtyShipmentDAOImp implements CreateDirtyShipmentDAO {
	...
	public List<Consumable> Get_Total_Consumables(String ids) {
		// TODO Auto-generated method stub
		int order_id=Integer.parseInt(ids);
		return (List<Consumable>)sessionFactory.getCurrentSession().createSQLQuery(" select pr.product_name , sum(CAST(op.product_quantity AS double precision)) as quantity , mu.unit as unit \r\n" + 
				"												from   products pr ,operations op,\r\n" + 
				"        measurment_unit mu  where \r\n" + 
				"									pr.product_sku = op.product_id         \r\n" + 
				"               \r\n" + 
				"        and CAST(mu.id AS VARCHAR) =CAST(pr.first_measurment_unit  AS VARCHAR)          \r\n" + 
				"        and op.transaction_id="+order_id+" \r\n" + 
				"        and op.transaction_movement_type='-1' \r\n" + 
				"        and transaction_type_id='3' \r\n" + 
				"        and pr.product_category in( 7,6)       \r\n" + 
				"    GROUP BY\r\n" + 
				"       \r\n" + 
				"        pr.product_name ,\r\n" + 
				"        mu.unit ,op.product_id			").setResultTransformer(Transformers.aliasToBean(Consumable.class)).list();
	}
	...
	public int Check_Destination_type_id(int shipment_id) {
		// TODO Auto-generated method stub
		List<Shipment> lst=(List<Shipment>)sessionFactory.getCurrentSession().createQuery("from Shipment where id="+shipment_id).list();
		if(lst!=null)
		{
		return lst.get(0).getShipment_destination_type_id();
		}
		else
		{
			return 0;
	...
	@SuppressWarnings("unchecked")
	@Override
	public String submitExternalShipment_(String driver, String user, String Lundry, String Products, int cat,
			String Source) {
		// TODO Auto-generated method stub
		ProcedureCall procedureCall = sessionFactory.getCurrentSession().createStoredProcedureCall("create_new_external_dirty_shipment");
		procedureCall.registerParameter("p_driver_user_name", String.class, ParameterMode.IN);
	...
	public int Check_Source_type_id(int shipment_id) {
		...
		if(lst!=null)
		{
		return lst.get(0).getShipment_source_type_id();
		}
		else
		{
			return 0;
