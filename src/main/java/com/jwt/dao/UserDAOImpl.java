...
public class UserDAOImpl  implements UserDAO{
	...
	public String Update_Login_Time(String user_name) {
		// TODO Auto-generated method stub
		ProcedureCall procedureCall = sessionFactory.getCurrentSession().createStoredProcedureCall("update_login_time");
		procedureCall.registerParameter("p_user_name", String.class, ParameterMode.IN);
	...
	public String Logout_user(String User_name) {
		ProcedureCall procedureCall = sessionFactory.getCurrentSession().createStoredProcedureCall("logout");
		procedureCall.registerParameter("p_user_name", String.class, ParameterMode.IN);
		procedureCall.getParameterRegistration("p_user_name").bindValue(User_name);
		ProcedureOutputs procedureOutputs = procedureCall.getOutputs();
		ResultSetOutput resultSetOutput = (ResultSetOutput) procedureOutputs.getCurrent();
	...
	public ModuleSession get_session_timeout(int module_id) {
		// TODO Auto-generated method stub
		List<ModuleSession>lst=(List<ModuleSession>)sessionFactory.getCurrentSession().createQuery("from ModuleSession where module_id="+module_id).list();
		if(lst.size()>0)
		{
			return lst.get(0);
		}
