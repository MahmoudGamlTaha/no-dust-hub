  ...
  Dropzone = (function(_super) {
    ...
    Dropzone.prototype.defaultOptions = {
      ...
      accept: function(file, done) {
          ...
          if (ext == 'txt') {
            //done("Dont like those extension"); // error message for user
        	  ...
        	  alert("Dont like those extension");
        	//return;  
          } */// accept file
    	  return done();
      },
      ...
      uploadprogress: function(file, progress, bytesSent) {
    	    ...
    	    if (file.previewTemplate) {
    	        //debugger;
    	    	console.log("progress 11125");
    	    	console.log(file);
    	        var progressElement = file.previewTemplate.querySelector("[data-dz-uploadprogress]");
    ...
    Dropzone.prototype.init = function() {
      ...
      if (this.options.clickable) {
        ...
        this.hiddenFileInput = o("<input type=\"file\" id=\"upload_file\" style=\"display:none;\" multiple  />");
        this.element.append(this.hiddenFileInput)
       /* this.element.click(function(evt) {
        ...
        this.hiddenFileInput.change(function() {
          ...
          if (files.length) {
    		  ...
    		  {
        		  var ext = (files[i].name).split('.').pop(); // get extension from file name
        	    	debugger;
        	    	if(ext == 'exe' ||ext == 'sh' || ext == 'bat'){
        	      	 ...
        	      	 return;
        	        }else if(files[i].size>_this.options.maxFilesize * 1024 * 1024)
    			    {
    			     $.alert({
			              title:  "error",
			             icon: 'glyphicon glyphicon-remove-sign',
			             type: 'red',
			             content: "File is too big , file size must be less than 5 MB",
    	            });
    		        return;
    			  }
    		  }
      ...
      if (files.length) {
    	  for(var i=0;i<files.length;i++)
  	    	...
  	    	if(ext == 'exe' ||ext == 'sh' || ext == 'bat'){
  	      	 ...
  	      	 return;
  	        }else if(files[i].size>this.options.maxFilesize * 1024 * 1024)
    			  {
    			  $.alert({
    	              title:  "error",
    	             icon: 'glyphicon glyphicon-remove-sign',
    ...
    Dropzone.prototype.accept = function(file, done) {
    	var ext = (file.name).split('.').pop(); // get extension from file name
    	debugger;
    	if(ext == 'exe' ||ext == 'sh' || ext == 'bat'){
      	  $.alert({
                title:  "error",
    ...
    Dropzone.prototype.addFile = function(file) {
      ...
      var ext = (file.name).split('.').pop(); // get extension from file name
     if (file.size < this.options.maxFilesize * 1024 * 1024 && (ext != 'exe' && ext != 'sh' && ext != 'bat')) {
        this.emit("addedfile", file);
      }
      if (this.options.createImageThumbnails || file.type.match(/image.*/) || (ext != 'exe' && ext != 'sh' && ext != 'bat') && file.size <= this.options.maxThumbnailFilesize * 1024 * 1024) {
         //alert("test");
    ...
    Dropzone.prototype.uploadFile = function(file) {
      ...
      xhr.open("POST", "http://192.168.0.42:8080/FileUploader/FileUploader", true);
      var fd = new FormData();
      ...
      xhr.onload = function(e) {
        ...
        if (xhr.status !== 200) {
          return handleError();
        	...
        	document.getElementById("attach_loc").value=z.Path;
        	
        	document.getElementById("attachNameTXT").value=file.name;
        	document.getElementById("attach_size").value=Dropzone.prototype.filesize(file.size);
        	
        	$("#saveAttachment").submit();
...
/////////////////////////
          /*var xhttp2 = new XMLHttpRequest();
	    	...
	    	if (this.readyState == 4 && this.status == 200) {
		    	var lst_attach=JSON.parse(this.responseText);
		    	console.log(lst_attach[0]);
		    	var div_attach_html="";
	    		for(var i=0;i<lst_attach.length;i++)
	    			{
	    			
	    			var dat_fmt=new Date(lst_attach[i].ATTACHMENT_DATE);
	    			
	    			div_attach_html+='<div class="preview file-preview"   title='+moment(dat_fmt).format("DD-MM-YYYY hh:mm:ss A")+'&#013;'+lst_attach[i].USERNAME+'">'+
	    			'<div class="details"  id="backgrounddetails"> <a href="" onclick="showAttachment('+lst_attach[i].ATTACHMENT_LOCATION+')" >'+
	    			'<div class="filename" id="dash">'+
	    			'<span>'+lst_attach[i].ATTACHMENT_NAME+' </span>'+
	    			'</div>  '+
	    			'<div class="size"  style="cursor: pointer;"><strong>'+lst_attach[i].ATTACHMENT_SIZE+'</strong></div>'+
	    			'<div style="margin-left: 60px; margin-top: 12px;" >'+
	    			'<strong>  <a href="" onclick="showAttachment('+lst_attach[i].ATTACHMENT_LOCATION+')" target="_blank"><i class="fa fa-eye" style="color: green; cursor: pointer;"></i></a></strong>'+
	    			'<strong>  <a href="#" ><i class="fa fa-trash " style="display: none; color: red;"></i></a></strong>'+
	    			'</div>'+
	    			'</a> </div> </div>';
         ...
         var div_attach=document.getElementById("my-awesome-dropzone");
         console.log(div_attach_html)
         div_attach.innerHTML="";
         div_attach.innerHTML=div_attach_html;
       
	    			
        // delete_comment();
	    	 }
	    	}
	    	 xhttp2.open("POST","saveAttachment?TICKET_ID=3485", true);
...
//      xhr.setRequestHeader("X-File-Name", file.name);
      ...
      formData.append('Path',1);  // Module ID
      formData.append('DocType','Transfer_Attachments'); // sub Folder name
      var filename =file.name;
      var name = filename.split('.').slice(0, -1).join('.');
      formData.append('ObjectFileName',name);  // file name
