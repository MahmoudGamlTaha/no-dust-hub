                       ...
                       <c:choose>
                        ...
                        ${shipment_details_data.shipment_Status}
                               ...
                               <c:if test="${shipment_details_data.type_id==5}">
                                <div class="row-fluid">
                                    <div style="width:45%;float:left""> <h5 style="margin-left:5px;"><strong><spring:message code="Drivername"/></strong>${shipment_details_data.driver_Name}	</h5></div>
                                  <div style="width:50%; float:left;"> <h5 "><strong>Product Type : </strong>${pr_type}	</h5></div> 
                             ...
                             <c:choose>
                               <c:when test="${shipment_details_data.type_id ==7||shipment_details_data.type_id ==3||shipment_details_data.type_id ==4||shipment_details_data.type_id ==8}">
                                   ...
                                   <div style="float:left; width:45%;">
                                       ...
                                       <div>
                                       <h5 style="display:none;"><strong>Bank Account Num : </strong>
						...
						<table id="shipmentTable" class="table table-striped table-bordered" style="width:80%; margin:auto;">
                                ...
                                <table id="shipmentTable" class="table table-striped table-bordered" style="width:80%; margin:auto;">
                                    ...
                                    <tr>
                                        ...
                                        <th><spring:message code="ProductName"/></th>
                                     <c:if test="${rec_ext!='Rec_External'}">  <th><spring:message code="ProductReconciliation.ProductTable.Quantity"/></th></c:if> 
                                       <th><spring:message code="receivedQuantity"/></th>
                                    <c:if test="${rec_ext!='Rec_External'}">    <th><spring:message code="Lost"/></th> </c:if>
                                   <c:if test="${rec_ext!='Rec_External'}">     <th><spring:message code="Over_"/></th> </c:if>
                                    </tr>
                                ...
                                <c:forEach items="${shipment_details_data.ship_details}" var="pro" varStatus="var_st">
                                    <tr>
                                        ...
                                        <td>${pro.product_name}</td>
                                        <td>${pro.quantity_shipped}</td>
                                       <c:if test="${rec_ext!='Rec_External'}">  <td>${pro.actual_reconcilied_quantity}</td></c:if>
                                       <c:if test="${rec_ext!='Rec_External'}">  <td><c:if test="${pro.lost>'0'}">${pro.lost}</c:if></td></c:if>
                                <c:if test="${rec_ext!='Rec_External'}">         <td><c:if test="${pro.over>'0'}">${pro.over}</c:if></td></c:if>
                                    </tr>
                           </c:forEach>
                                    </tbody>
                                </table >
	  	 ...
	  	 if(validate_form!=false)
      		...
      		xhttp.onreadystatechange = function() {
      			...
      			document.getElementById("printBtn").disabled=false;
      				...
      				{
      				swal("<spring:message code="ShipmentList.Error.Title"/>","<spring:message code ="Shipmentclosed"/>","error");
      				/*$.alert({
      				...
      				{
      				swal("<spring:message code="ShipmentList.Error.Title"/>","<spring:message code="ShipmentList.Error.content"/>","error");
      				/*$.alert({
    		              title: '<spring:message code="ShipmentList.Error.Title"/>',
