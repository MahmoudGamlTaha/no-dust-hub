...
angular.module('myApp').controller('DraftController', ['$scope', 'DraftService', function($scope, DraftService) {
   ...
   self.detailTicket=detailTicket
 ...
 //   fetchCS();
  // fetchDirection();
   ...
   {
	   DraftService.fetchAllDirection()
       .then(
       function(d) {
        console.log(d);
          ...
          if(d.length<=0)
        	  {
      	$('#src_Direction').multiselect({nonSelectedText:"Direction"});
      	$('#src_Direction').multiselect('refresh');
        	  }
       },
       function(errResponse){
   ...
   function fetchAllCase()
   {
    ...
    function ShowAllticket()
    {
	   ...
	   var div_search = document.getElementById("div_search");
		  ...
		  document.getElementById("src_txt").value="";


    }
   fetchMYDraftTicket();
    function fetchMYDraftTicket(){
    	DraftService.fetchMYDraftTicket()
            .then(
            function(d) {
    ...
    {
    	DraftService.fetchAllDraftTicket(query)
        .then(
        function(d) {
        	 //$("#Mytable").DataTable().clear();
   		  $("#AllTable").DataTable().destroy();
    ...
    function fetchUnAssignDraftTicket()
    {
    	...
    	DraftService.fetchUnAssignDraftTicket()
        .then(
        function(d) {
        	 //$("#Mytable").DataTable().clear();
		    ...
		    .appendTo( '#UnAssigntable_wrapper .col-md-6:eq(0)' );
        	  ...
        	  self.MyDraft=false;
        	   self.UnAssign=true;
        	   self.AllTicket=false;
        	   self.title="UnAssign Draft Ticket";
        	   var div_search = document.getElementById("div_search");
        		  div_search.style.display = "block";
        		  document.getElementById("src_txt").value="";
    ...
    function SearchUnAssignDraftTicket()
    {
    	...
    	DraftService.SearchUnAssignDraftTicket(query)
        .then(
        function(d) {
        	 $("#UnAssigntable").DataTable().destroy();
   		  //$("#Mytable").DataTable().destroy();
    ...
    function detailTicket(id)
    {
    	...
    	var encryptedAES = CryptoJS.AES.encrypt(id.toString(), "My Secret Passphrase");  
    	console.log(encryptedAES);
    	console.log(encryptedAES.toString());
    	var dec = CryptoJS.AES.decrypt(encryptedAES.toString(), "My Secret Passphrase");
    	console.log(dec);
    	window.location="detailTicket?id="+encryptedAES.toString();
    console.log(CryptoJS.enc.Utf8.stringify(dec).toString());
    	}
